...
//! Just some typedef for Eigen objects.
typedef Eigen::Matrix<number, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> Matrix;
typedef Eigen::Matrix<number, Eigen::Dynamic, 1 > Vector;

//! A generic iterator that works with Eigen containers
/*! Code found at https://forum.kde.org/viewtopic.php?f=74&t=94120
 */
...
public:
        ...
        friend index_iterator operator+(const index_iterator & a, int b)
        {
                index_iterator ret(a);
                ret += b;
                return ret;
        }
        friend index_iterator operator-(const index_iterator & a, int b)
        ...
        friend index_iterator operator+(int a, const index_iterator & b)
        {
                index_iterator ret(b);
                ret += a;
                return ret;
        }
        friend index_iterator operator-(int a, const index_iterator & b)
        ...
        int operator-(const index_iterator & other) const
        {
